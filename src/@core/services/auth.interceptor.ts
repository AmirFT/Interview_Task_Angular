...
export class AuthInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        if (accessToken) {
            // if (this.tokenStoreService.hasStoredApartmentToken()) {
            //     request = request.clone({
            //         headers: request.headers
            //             .set(this.authorizationHeader, `Bearer ${accessToken}`)
            //             .set(this.languageHeader, this.getCurrentLanguage())
            //             // .set(this.apIdHeader, this.tokenStoreService.getAuthUserAptID())
            //     });
            // } else {
                request = request.clone({
                    headers: request.headers
    ...
    getNewAuthRequest(request: HttpRequest<any>): HttpRequest<any> | null {
        ...
        if (requestAccessTokenHeader === newAccessTokenHeader) {
            console.log('There is no new AccessToken.', { requestAccessTokenHeader, newAccessTokenHeader });
            return null;
        }
    ...
    private getCurrentLanguage(): string {
        const lang = this.tokenStoreService.hasStoredConfigToken() ? this.tokenStoreService.getConfigToken().lang : 'fa';
        switch (lang) {
            case 'en':
                return 'en-US';
            case 'fa':
